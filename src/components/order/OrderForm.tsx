      ...
      <div className="space-y-5">
            ...
            {products.map((p) => (
              ...
              >
                ...
                <span className="flex items-center gap-1.5">
                  <span className="tabular me-auto text-sm font-bold text-brand-700">{formatUsd(p.price_cents, locale.intl)}</span>
                  {inOrder.has(p.id) && (
                    <span
                      key={inOrder.get(p.id)}
                      title={t("order.inOrderLabel", { count: inOrder.get(p.id) ?? 0 })}
                      className="tabular animate-pop rounded-full bg-sun-400 px-2 py-0.5 text-xs font-bold text-brand-900"
                    >
                      {t("order.inOrder", { count: inOrder.get(p.id) ?? 0 })}
                      <span className="sr-only"> {t("order.inOrderLabel", { count: inOrder.get(p.id) ?? 0 })}</span>
                    </span>
                  )}
        ...
        <Card>
          ...
          <p id="rate-hint" className="mt-2 flex items-center gap-2 text-xs text-neutral-600">
            <span className="shrink-0 whitespace-nowrap rounded-full bg-neutral-100 px-2 py-0.5 font-semibold">{t("order.minChip", { min: minRateText })}</span>
